...
def main():
    ...
    if len(train_data) == 0 or len(val_data) == 0 or len(test_dataset) == 0:
    ...
    if model_path != "": # If a model_path is provided, train the model from a checkpoint
        model = load_classifier_model(model_path, selected_genera)
    elif model_path == "": # Else if model_path is empty: train the model from scratch
